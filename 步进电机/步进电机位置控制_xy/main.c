...
//步进电机脉冲发射
void step_motor_send_pulse(uchar step,uchar dir,uchar mode)
{
	uchar temp=step;	
	if(dir==0)	//反转节拍调换
		temp=3-step;
	...
	{	
		switch(temp)//4拍控制：A->B->C->D
		{
			case 0: IN2_A=1;IN2_B=0;IN2_C=0;IN2_D=0;break;
			case 1: IN2_A=0;IN2_B=1;IN2_C=0;IN2_D=0;break;
			case 2: IN2_A=0;IN2_B=0;IN2_C=1;IN2_D=0;break;
			case 3: IN2_A=0;IN2_B=0;IN2_C=0;IN2_D=1;break;
			default: IN2_A=0;IN2_B=0;IN2_C=0;IN2_D=0;break;	
		}
	}			
}
...
void int2str(float a)
{
	...
	if(mode==1)
	{miley_str[0]=tra[baiwei];miley_str[1]=tra[shiwei];miley_str[2]=tra[gewei];miley_str[3]='.';miley_str[4]=tra[shifengwei];miley_str[5]=tra[baifengwei];}
}
...
{		
	...
	while(1)
	{
		...
		if (key_press_value!=-1&&milex>=0&&milex<=200&&miley>=0&&miley<=200)
		{
			if (key_press_value==0)
			{MOTOR1_EA=1;MOTOR2_EA=1;dir=1;LED_FWD=1;LED_REV=0;}
			else if (key_press_value==1)
			{MOTOR1_EA=1;MOTOR2_EA=1;dir=0;LED_FWD=0;LED_REV=1;}
...
void bujujiao(void) interrupt 1
{
	...
	if(mode==0)
	{
		(step0)%=4;step_motor_send_pulse((step0)++,dir,0);
		if(dir==0){milex+=1.05;}
		if(dir==1){milex-=1.05;}
	}
	else if(mode==1)
	{
		(step1)%=4;step_motor_send_pulse((step1)++,dir,1);
		if(dir==0){miley+=1.05;}
		if(dir==1){miley-=1.05;}
	}
	num_plus++;
...
void LCD_SHOW(void) interrupt 3
{
	...
	TL1 = 0XB0;
	if(mode==0)
	{if(dir==0){lcd_show_string(0,0,"X+");}else if(dir==1){lcd_show_string(0,0,"X-");}}
	else if(mode==1)
	{if(dir==0){lcd_show_string(0,0,"Y+");}else if(dir==1){lcd_show_string(0,0,"Y-");}}
	int2str(milex);
